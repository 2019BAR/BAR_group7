...
```{r}
table(USDA$HighSodium) #高鈉分類下的數目
```

```{r}
table(USDA$HighSodium, USDA$HighFat) #高鈉分類和高熱量分類的交叉表
```

```{r}
tapply(USDA$Iron, USDA$HighProtein, mean, na.rm=TRUE)#根據高蛋白質分類鐵質的平均，遺落值可以忽略
```

```{r}
tapply(USDA$VitaminC, USDA$HighCarbs, max, na.rm=TRUE)#根據高碳水分類維他命C的最大值，遺落值可以忽略
```

```{r}
tapply(USDA$VitaminC, USDA$HighCarbs, summary, na.rm=TRUE)#根據高碳水化合物分類維他命C的總和，遺落值可以忽略
```
<br>
