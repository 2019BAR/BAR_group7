...
min(IBM$Date,GE$Date,PG$Date,CO$Date,BOE$Date)
```

__§ 1.3__ What is the latest year in our datasets?
```{r}
max(IBM$Date,GE$Date,PG$Date,CO$Date,BOE$Date)
range(IBM$Date,GE$Date,PG$Date,CO$Date,BOE$Date) # 補充驗證，列出時間區間的起始與結束
```
<br>
...
```{r}
lapply(L, function(df) range(df$Date)) #lapply(x, fun)
```
...
```{r}
plot(CO$Date, CO$StockPrice, type="l") #type="l"表示畫線圖
```
```{r}
CO$Date[which.max(CO$StockPrice)]  # 股價最高時期
```
```{r}
CO$Date[which.min(CO$StockPrice)]  # 股價最低時期
```
...
lines(BOE$Date, BOE$StockPrice, type="l", col="cyan")
lines(GE$Date, GE$StockPrice, type="l", col="pink")
lines(CO$Date, CO$StockPrice, type="l", col="purple")
lines(PG$Date, PG$StockPrice, type="l", col="blue")
abline(v=as.Date(c("2000-03-01","1983-01-01", "1984-01-01")),col="red",lty=3, lwd=2)
```
```{r}
# 由上圖畫線判斷BOE為當時最低
```
...
```{r}
plot(CO$Date, CO$StockPrice, type="l", col="purple", xlim =as.Date(c("1983-01-01","1984-01-01")))
lines(PG$Date, PG$StockPrice, type="l", col="blue")
# 由上圖畫線判斷
# Coca-Cola為當時上漲
# Procter and Gamble 下跌
```
...
```{r}
plot(IBM$Date, IBM$StockPrice, type="l", col="orange", ylim = c(0,250),xlim =as.Date(c("1995-01-01","2005-12-01")))
lines(BOE$Date, BOE$StockPrice, type="l", col="cyan")
lines(GE$Date, GE$StockPrice, type="l", col="pink")
lines(CO$Date, CO$StockPrice, type="l", col="purple")
lines(PG$Date, PG$StockPrice, type="l", col="blue")
...
```{r}
 IBM$Date %in% as.Date(c("1997-09-01", "1997-11-01"))
