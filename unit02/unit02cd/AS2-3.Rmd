...
```{r}
table(A$Industry) %>% sort %>% head
```

Which state has the largest number of interviewees?
```{r}
table(A$Industry) %>% sort %>% tail
```
...
```{r}
tapply(A$MetroAreaCode, A$State, mean) %>% sort 
```
...
```{r}
tapply(is.na(A$MetroAreaCode), A$State, mean) %>% sort #Wisconsin 
```


Which state has the largest proportion of non-metropolitan interviewees, ignoring states where all interviewees were non-metropolitan?
```{r}
tapply(is.na(A$MetroAreaCode), A$State, mean) %>% sort #Montana 
