...
```{r}
subset(df, height > 170 & !isMale)$name # 身高大於170且為女生的名字
```

```{r}
mean(df$height[df$isMale]) # 所有男生的平均身高
```

```{r}
df$height[!df$isMale] %>% mean # 女生身高取平均值
```

```{r}
sum( subset(df, !isMale)$noBuy ) # 所有女生購買次數總和
```

```{r}
subset(df, skin_color == "white" & !isMale ) %>% nrow #白種人且為女生的列數
```

```{r}
sum(df$skin_color == "white" & !df$isMale ) # 白皮膚女性總數
```
