...
<z>二維的`table()`</z>
```{r}
table(format(ts,'%w')) # 從星期天到星期六每天發生竊案的數量
```
```{r}
table(format(ts,'%m'))
```
...
```{r}
table(ts > as.POSIXct("2007-01-01")) #20070101區分總時間為前後時間段，前半段較多人被逮捕
```
